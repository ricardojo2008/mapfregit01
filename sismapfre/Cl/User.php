...
class Cl_User
{
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT user_id, name, email, created,social_id,(SUBSTRING_INDEX(email, '@',1)) as corto FROM users where (email = '$email' or SUBSTRING_INDEX(email, '@',1)='$email' ) and password = '$password' ";
			//echo $query;
	...
	public function forgetPassword( array $data )
	{
		if( !empty( $data ) ){
			...
			if(mysqli_query($this->_con, $query)){
				mysqli_close($this->_con);
				$to = $email;
				$subject = "Se ha generado una Nueva solicitud de contraseña";
				$txt = "Le informamos que su nueva contraseña ".$password;
				$headers = "From: tec03@mapfre.com.ar" . "\r\n" .
						"CC: tec03@mapfre.com.ar";
